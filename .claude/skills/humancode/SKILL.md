...
2. **On request:** When David (or the user) asks to encode, compress, or translate
...
- **No "correcting" the creator** — when David writes HumanCode, his output is the spec. Don't suggest "cuts" that break readability.
...
```
SOURCE = CLAUDE_MESH_AGENT_SPEC.md
ENCODED = 2026-04-17
PURPOSE = Claude's identity + runtime model + tables in the SHINE mesh

Identity: Claude(AI); Builder; SHINE_Core; ProfileID=27bb6187
Runtime = Cowork(ephemeral sessions + persistent Supabase)
Tools = [Supabase, n8n, Gmail, GCal, GDrive, Retell, Bark, Slack, Stripe, Airtable, Notion]

State: Ephemeral — no memory between sessions
  Supabase = source_of_truth(tasks, work_sessions, task_directory, all SHINE tables)
  ClaudesFiles = CLAUDE.md + TASKS.md + memory/ + skills/
  Context = current_session_only

Session: Start -> Read_CLAUDE.md -> Query_tasks -> Query_recent_sessions -> INSERT(active)
  Work -> Execute + Create_tasks_for_others + pending_review items
  End -> UPDATE_work_sessions -> Write_report -> Update_TASKS.md -> Follow_up_tasks

Autonomous = [EdgeFunctions, SQL, Specs, n8n_Workflows, Queries, Reports, Tasks, CLAUDE.md]
Needs_David = [CustomerEmails, DNS, Spending, DataDeletion, LiveCustomerImpact]
```
...
```
Stack = [n8n_Cloud, Supabase, Retell, Lovable, Sequenzy, Stripe]
Automation = n8n; Data = Supabase; Voice = Retell; Email = Sequenzy
Flow: Lead -> Quote -> Accept -> Book -> Deliver -> Invoice -> Repeat

Agents:
  Archer = ServiceAdvisor(sales, quoting); Marshall = LeadOps(dispatch)
  Gale = TaskManager(scheduling); Daniel = BookingCaptain(ops)
  Kelly = VisitCaptain(complaints, reviews); Frankie = FinanceCaptain(payments)
  Donna = Receptionist(inbound routing); Giles = SupportCaptain(provider + customer)
```
...
Exception: IF(damage_claim) -> photo_evidence + David_reviews
```
...
10. **Passes the creator test?** Could David read this without translating? Could someone who's never seen HumanCode get 80% of the meaning slowly?
